...
export default function DashboardPage() {
  ...
  useEffect(() => {
    // Check if user is new and hasn't seen the tour this session
    const hasSeenTour = localStorage.getItem("hasSeenLogicLensTour");
    if (traces.length === 0 && favorites.length === 0 && !hasSeenTour) {
      const timer = setTimeout(() => setShowTour(true), 1500);
      return () => clearTimeout(timer);
    }
  }, [traces.length, favorites.length]);

  const handleCloseTour = () => {
    setShowTour(false);
    localStorage.setItem("hasSeenLogicLensTour", "true");
  };
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            <div>
              <h1 className="text-2xl font-bold mb-1">
                ...
                {user?.planType === "pro" && (
                  <Badge className="ml-2 bg-primary/10 text-primary border-primary/20 gap-1 animate-pulse">
