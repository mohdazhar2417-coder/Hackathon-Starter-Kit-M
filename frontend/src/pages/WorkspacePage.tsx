...
export default function WorkspacePage() {
  ...
  return (
    <div className="h-[calc(100vh-56px)] bg-background flex flex-col overflow-hidden">
      {/* Dynamic Header */}
      <div className="h-14 border-b border-border bg-card/80 backdrop-blur-md px-6 flex items-center justify-between shrink-0 z-30">
          ...
          {selectedProgram && (
            <div className="flex flex-col">
              <div className="flex items-center gap-3">
                <span className="text-sm font-black tracking-tight uppercase">{selectedProgram.name}</span>
                <Badge variant="outline" className={cn("text-[9px] uppercase font-black", DIFFICULTY_BADGE[selectedProgram.difficulty])}>
                  {selectedProgram.difficulty}
                </Badge>
              </div>
              <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest">{selectedProgram.category}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button 
            ...
            size="sm" 
            className={cn(
              "h-10 w-10 p-0 rounded-xl border-white/10 bg-[#1a1a2e] backdrop-blur-md transition-all hover:bg-[#252545]",
              autoPlay ? "text-primary border-primary/30" : "text-white/60"
            )}
            onClick={() => setAutoPlay(!autoPlay)}
          >
            {autoPlay ? <Pause className="h-4 w-4" /> : <Orbit className="h-4 w-4" />}
          </Button>
          ...
          <Button 
            variant="outline"
            size="sm"
            className="h-10 w-10 p-0 rounded-xl border-white/10 bg-[#1a1a2e] text-white/60 hover:bg-[#252545] transition-all"
            onClick={() => goToStep(0)}
          ...
          <Button 
            variant="outline"
            size="sm" 
            className="h-10 px-4 rounded-xl border-white/10 bg-[#1a1a2e] text-white/80 gap-2 font-semibold text-xs hover:bg-[#252545] transition-all"
            onClick={() => prevStep()}
            disabled={!simResult || activeStep === 0}
          >
          ...
          {/* Step Forward - highlighted */}
          <Button 
            size="sm" 
            className="h-10 px-5 rounded-xl bg-primary text-primary-foreground gap-2 font-semibold text-xs shadow-lg shadow-primary/25 hover:bg-primary/90 transition-all"
            onClick={() => nextStep()}
          ...
          <div className="flex items-center gap-1.5">
            <Button
              ...
              size="sm"
              className={cn(
                "h-10 w-10 p-0 rounded-xl border-white/10 bg-[#1a1a2e] transition-all",
                favorites.some(f => f.programId === selectedProgram?.id) ? "text-pink-500 border-pink-500/30 bg-pink-500/5" : "text-white/60 hover:bg-[#252545]"
              )}
              onClick={() => {
                if (!selectedProgram) return;
            ...
            >
              <Heart className={cn("h-4 w-4", favorites.some(f => f.programId === selectedProgram?.id) && "fill-current")} />
            </Button>
            ...
            <Button
              ...
              onClick={() => {
                ...
                createTrace.mutate({
                  ...
                  onSuccess: () => {
                    setIsSaving(false);
                    toast({ title: "Trace Saved", description: "You can find this in your 'Recent Saves' dashboard." });
                    refetchTraces();
                  },
            ...
            >
              {isSaving ? <RefreshCw className="h-3.5 w-3.5 animate-spin" /> : <BookMarked className="h-3.5 w-3.5" />}
              Save
            </Button>
            ...
            <Button
              variant="outline"
              size="sm"
              className="h-10 w-10 p-0 rounded-xl border-white/10 bg-[#1a1a2e] text-primary hover:bg-primary/5 transition-all"
              onClick={() => {
                if (!selectedProgram || !simResult) return;
                ...
                const publishAndCopy = (id: number) => {
                  ...
                  .then(data => {
                    ...
                    toast({ title: "Link Copied!", description: "Anyone with this link can view your logic trace." });
                  });
                };
          ...
          <ResizablePanelGroup direction="vertical" className="flex-1">
            <ResizablePanel defaultSize={75} minSize={30} className="relative">
              ...
              >
                ...
                <div className={cn(
                  ...
                  <div className="p-3 bg-muted/50 border-b border-border flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col">
                        <span className="text-[10px] font-black uppercase tracking-widest leading-none">Code View</span>
                        {selectedProgram && (
                          <span className="text-[8px] text-muted-foreground font-mono mt-0.5">{selectedProgram.name}.java</span>
                        )}
                      </div>
                    </div>
                    <Button variant="ghost" size="sm" className="h-6 w-6 p-0 hover:bg-white/5" onClick={() => setShowEditor(!showEditor)}>
                      {showEditor ? <Minimize2 className="h-3 w-3" /> : <Maximize2 className="h-3 w-3" />}
                    </Button>
                  </div>
                  {showEditor && (
                    <div 
                ...
                {currentStep && (
                  <div className="bg-card/80 backdrop-blur-xl border border-border rounded-2xl shadow-2xl p-4 animate-in slide-in-from-right duration-500">
                    <div className="flex items-center gap-2 mb-3">
                      <Info className="h-4 w-4 text-primary" />
                      <span className="text-[10px] font-black uppercase tracking-widest">Execution Info</span>
                    </div>
                    <div className="space-y-3">
                      <p className="text-xs font-bold leading-relaxed">{currentStep.description}</p>
                      <div className="p-2 rounded-lg bg-yellow-500/5 border border-yellow-500/20">
                        <p className="text-[10px] text-yellow-700 leading-relaxed italic">
                          <span className="font-black">TIP:</span> {currentStep.type === 'assignment' ? "Watch how the value updates instantly in the node above." : "The flow follows the true/false logic shown in the decision node."}
                        </p>
                      </div>
                    </div>
                  </div>
